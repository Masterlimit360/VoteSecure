...
import { Fingerprint, ShieldCheck, ScanFace, Sparkles, AlertCircle, CheckCircle2, RefreshCw, KeyRound, Camera, PowerOff } from 'lucide-react';
...
const Login = () => {
  ...
  // Default to 'voter' credential login so it doesn't auto-scan face immediately
  const [loginMode, setLoginMode] = useState<'voter' | 'face' | 'admin'>('voter');
  ...
  const handleFaceLogin = useCallback(async (imageSrc: string) => {
    ...
    try {
      ...
      const errorMsg =
        ...
        'Face verification failed. Please ensure good lighting, look directly at the camera, or log in with your password.';
      setError(errorMsg);
    } finally {
      setLoading(false);
  ...
  const { feedback, isValid, reset } = useFaceScanner(
    ...
    canvasRef,
    handleFaceLogin,
    loginMode === 'face' && isCameraActive && !loading && !successInfo
  );
  ...
  const handleModeChange = (mode: 'voter' | 'face' | 'admin') => {
    setLoginMode(mode);
  ...
  return (
    <div className="max-w-lg w-full mx-auto space-y-8 p-6 sm:p-10 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 transition-all">
      ...
      <div className="flex bg-gray-100 dark:bg-gray-800/80 p-1.5 rounded-2xl gap-1 border border-gray-200/50 dark:border-gray-700/50">
        <button
          type="button"
          onClick={() => handleModeChange('voter')}
      ...
      {/* 1. Voter Password Form (Default Tab) */}
      {loginMode === 'voter' && (
      ...
      {/* 2. Biometric Face Scan Mode (Requires explicit activation) */}
      {loginMode === 'face' && (
        <div className="space-y-5 text-center">
          ...
          {!isCameraActive ? (
            /* Camera Inactive State — User must click to start */
            <div className="border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-3xl p-8 flex flex-col items-center justify-center space-y-4 bg-gray-50/50 dark:bg-gray-800/30">
              <div className="w-20 h-20 rounded-full bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-inner">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
                  Click below to activate the camera and verify your enrolled biometric face identity.
                </p>
              </div>
              ...
              <button
                ...
                className="mt-2 py-3.5 px-6 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold rounded-2xl shadow-lg shadow-emerald-600/25 transition-all text-sm flex items-center justify-center gap-2 transform active:scale-95"
              >
            ...
            <div className="space-y-5">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
                Position your face squarely in the frame. Hold still for automatic recognition, or click Scan below.
              </p>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="py-3 px-3 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-semibold rounded-xl transition-colors text-sm flex items-center justify-center gap-1.5"
                  title="Reset Scanner"
                >
                ...
                <button
                  ...
                  className="py-3 px-3 bg-red-50 hover:bg-red-100 dark:bg-red-950/40 dark:hover:bg-red-900/40 text-red-600 dark:text-red-400 font-semibold rounded-xl transition-colors text-sm flex items-center justify-center gap-1.5"
